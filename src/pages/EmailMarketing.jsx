import React, { useState } from 'react';
import { base44 } from '@/api/base44Client';
import { Card } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Textarea } from '@/components/ui/textarea';
import { Dialog, DialogContent, DialogHeader, DialogTitle } from '@/components/ui/dialog';
import { Loader, Plus, Mail } from 'lucide-react';
import { toast } from 'sonner';

export default function EmailMarketing() {
  const [showDialog, setShowDialog] = useState(false);
  const [loading, setLoading] = useState(false);
  const [campaignData, setCampaignData] = useState({
    campaign_name: '',
    subject_line: '',
    html_content: ''
  });

  const handleCreate = async () => {
    if (!campaignData.campaign_name || !campaignData.subject_line) {
      toast.error('Name und Betreff erforderlich');
      return;
    }

    setLoading(true);
    try {
      await base44.functions.invoke('createEmailCampaign', campaignData);
      setShowDialog(false);
      setCampaignData({ campaign_name: '', subject_line: '', html_content: '' });
      toast.success('Kampagne erstellt');
    } catch (error) {
      toast.error('Fehler');
    } finally {
      setLoading(false);
    }
  };

  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold text-gray-900">Email Marketing</h1>
        <Button onClick={() => setShowDialog(true)} className="gap-2">
          <Plus className="w-4 h-4" />
          Kampagne
        </Button>
      </div>

      <Card className="p-6">
        <p className="text-gray-600">Erstelle und verwalte Email-Kampagnen</p>
      </Card>

      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Neue Kampagne</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-1">Kampagnenname</label>
              <Input
                value={campaignData.campaign_name}
                onChange={(e) => setCampaignData({ ...campaignData, campaign_name: e.target.value })}
              />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Betreff</label>
              <Input
                value={campaignData.subject_line}
                onChange={(e) => setCampaignData({ ...campaignData, subject_line: e.target.value })}
              />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Content</label>
              <Textarea
                value={campaignData.html_content}
                onChange={(e) => setCampaignData({ ...campaignData, html_content: e.target.value })}
                rows={5}
              />
            </div>
            <Button onClick={handleCreate} disabled={loading} className="w-full gap-2">
              {loading && <Loader className="w-4 h-4 animate-spin" />}
              Erstellen
            </Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
  );
}